/*
  LocalizaBus — api/semob-stops.js
  Proxy serverless para carregar paradas da SEMOB.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const response = await fetch(MOBILIBUS_STOPS_URL, {
    ...
    return res.status(500).json({
      ...
      error: error?.name === 'AbortError'
        ? 'Tempo limite ao buscar paradas SEMOB/Mobilibus'
        : error?.message || 'Erro ao buscar paradas SEMOB/Mobilibus',
    });
  }
}
